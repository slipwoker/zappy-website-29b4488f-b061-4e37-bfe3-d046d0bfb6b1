document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function validateField(field) {
      ...
      if (type === 'tel' && !VALIDATORS.tel(value)) {
        showFieldError(field, getErrorMessage('tel'));
        return false;
      }

      const minLength = field.getAttribute('minlength');
      if (minLength && !VALIDATORS.minlength(value, minLength)) {
        showFieldError(field, getErrorMessage('minlength', minLength));
        return false;
      }
  ...
  // ============================================================
  const ANIMATION_CLASSES = ['fade-in', 'fade-in-up', 'fade-in-down', 'fade-in-left', 'fade-in-right', 'animate-on-scroll'];
  const VISIBLE_CLASS = 'is-visible';
  const ANIMATION_THRESHOLD = 0.15;
  const ANIMATION_ROOT_MARGIN = '0px 0px -50px 0px';

  function collectAnimatableElements() {
    const selectors = ANIMATION_CLASSES.map(function (cls) {
      return '.' + cls;
    }).join(', ');
  ...
  function initScrollAnimations() {
    const elements = collectAnimatableElements();
  ...
  const mutationObserver = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
      mutation.addedNodes.forEach(function (node) {
        ...
        ANIMATION_CLASSES.forEach(function (cls) {
          if (node.classList && node.classList.contains(cls) && !node.classList.contains(VISIBLE_CLASS)) {
            initScrollAnimations();
          }
          const children = node.querySelectorAll ? node.querySelectorAll('.' + cls) : [];
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "רשת כוללים  שערי מרדכי משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '29b4488f-b061-4e37-bfe3-d046d0bfb6b1',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '29b4488f-b061-4e37-bfe3-d046d0bfb6b1';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '29b4488f-b061-4e37-bfe3-d046d0bfb6b1';
